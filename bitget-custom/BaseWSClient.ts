...
} from 'bitget-api/lib/types/index'
import { DefaultLogger } from 'bitget-api/lib/util/logger'
import { isWsPong } from 'bitget-api/lib/util/requestUtils'
import { getWsAuthSignature } from 'bitget-api/lib/util/websocket-util'
...
export interface IBaseWebsocketClient<TWSKey extends string> {
  ...
  implements IBaseWebsocketClient<TWSKey>
{
  ...
  public subscribe(
    ...
    const wsKey = this.getWsKeyForTopic(topics[0], isPrivateTopic)

    // Persist this topic to the expected topics list
    this.wsStore.addTopic(wsKey, topics)
  ...
  public unsubscribe(
    ...
    this.wsStore.deleteTopic(
      this.getWsKeyForTopic(topics[0], isPrivateTopic),
      topics,
    )
    ...
    if (
      ...
      this.requestUnsubscribeTopics(wsKey, [topics[0]])
    }
  }
  ...
  private async onWsOpen(event: WebSocket.Event, wsKey: TWSKey) {
    ...
    const topics = [...this.wsStore.getTopics(wsKey)].flat()
  ...
  private onWsMessage(event: any, wsKey: TWSKey) {
    try {
