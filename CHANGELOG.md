...
- A price feed that stays dead now gets a fresh worker within a few minutes. When a feed stalls, the worker rebuilds its connector in place, which covers short interruptions. But the rebuild runs in the same thread and keeps whatever that thread holds, so a feed it could not revive stayed dead until the next scheduled process restart. The escalation that was meant to catch this could never fire, because its counter was reset to the value it compared against and then incremented past it. The worker now counts consecutive stalls per feed, resets the count on the first real tick, and exits after three stalls with no data between them. Its supervisor then starts a fresh thread. The in-place reconnect for short interruptions and the scheduled restart are unchanged.
...
- The inverse perpetuals' tickers are actually opened. They were matched against the venue's classic contract listing, which no longer carries them at all, so the match was always empty and no ticker was subscribed. They are taken from the venue's unified listing instead.
...
- **Closing a Binance spot Ed25519 user stream now also drops a resubscription the client library had already queued.** 1.14.17 added this, but the stream's close path was left calling the underlying socket close directly, so a resubscription queued just before a stream was closed could still reopen its connection. The close path now goes through the stream wrapper. Spec 008 §1.1.3.
...
- **A failed Hyperliquid ticker subscription spun a zero-delay restart loop that starved the entire price worker's event loop.** `initHyperliquidWS()`'s `catch` called `hyperliquidRestartCb()` directly, which calls `initHyperliquidWS()` again — no delay, no backoff, no in-flight guard. While Hyperliquid rate-limits the connector the `allMids` subscribe fails persistently, so the two functions called each other in a tight loop; and because each iteration tears down and rebuilds every `WebSocketTransport`, the loop kept re-tripping the per-IP connection limit that caused it. In the field this dominated the price connector's error output, with a flood of abnormal socket closes and runaway CPU and memory use. Critically the loop re-enters through *already-settled* promises, so every iteration is a microtask and the macrotask queue never gets a turn: a driven A/B measured **578 149 subscribe attempts and 1 156 296 transports in 100s, with a 1s `setInterval` firing 0 times** — i.e. `CommonConnector.watchdogFn` and every other exchange's timers were stopped too, which is the real source of the collateral `bitgetUsdm` stall crashes seen in the same runs. All full restarts now go through a single-flight `scheduleHyperliquidRestart()`: concurrent requests coalesce into the pending restart, and the delay is `backoff − elapsed` (30s, doubling to a 5min cap, reset to base after 10min quiet) so a restart following a healthy period still fires **immediately** and only consecutive failures are paced. Same A/B after the fix: **4 attempts / 6 transports in 100s, 99 of 100 timer ticks**.
...
- OKX Europe X-Perp ticker feed: a dedicated, isolated WS client subscribed only to xperp instruments publishes them on the canonical `trade@{pair}@okxLinear` channels (expiry suffix stripped to the `BTC-USD_UM_XPERP` pair id). Optional `OKX_EU_ONLY` mode for EU-only deployments. Contributed by a community member.
...
### Added
- Price/candle data-stall observability: the price connector's per-channel watchdog now publishes a `{ watchdogStall }` event to the `serviceLog` Redis channel before it self-restarts, so the otherwise-silent recovery is visible to consumers of that channel.
